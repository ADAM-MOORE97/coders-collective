import React from "react";
import styled from "styled-components";

function BlogCard({
  fetchPatch,
  user,
  user: { blog_link, pictureUrl = "data:image/jpeg;base64,", username, likes },
}) {
  function handleLikeBtn(user) {
    fetchPatch(user);
  }

  return (
    <CardContainer>
      <h2>{username}</h2>
      <img alt="No image found" src={pictureUrl} />
      <p>{username}'s Blog:</p>
      <a href={blog_link} target="blank">
        {blog_link}
      </a>
      <br />
      <button onClick={() => handleLikeBtn(user)}>Likes:{likes}</button>
    </CardContainer>
  );
}

export default BlogCard;

const CardContainer = styled.div`
  transition: 0.3s;
  border-radius: 15px 15px 15px 5px;
  width: 400px;
  height: 425px;
  background-color: white;
  div:hover {
    box-shadow: 10 8px 16px 0 rgba(0, 0, 0, 0.2);
  }
`;

